...
#define TABSIZE (sizeof(predef)/sizeof(char *))
...
output_prefix(void)
{
	...
	int i;
        for (i = 0; i < TABSIZE; i++) {
        	outline++;
        	fprintf(code_file, predef[i], symbol_prefix);
        }
    }
    ++outline;
...
pack_vector(int vector)
{
	...
	for (k = 0; ok && k < t; k++) {
		...
		table = (short *) erealloc(table, newmax * sizeof(short));
		check = (short *) erealloc(check, newmax * sizeof(short));
...
output_debug(void)
{
    ...
    symnam = (char **) emalloc((max + 1) * sizeof(char *));

 /*
  * Note that it is  not necessary to initialize
  * the element
  */
 /* symnam[max].							 */
    for (i = 0; i < max; ++i)
